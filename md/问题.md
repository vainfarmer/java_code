...
/**
  ...
  @Query(value = """
      SELECT
        ...
        a.answer,
        a.feedback,
        a.feedback_reason AS feedbackReason,
        a.user,
        a.message_ctime AS ctime
      FROM feedback_cycle_tab a
      INNER JOIN topic_message_relation_tab b
      ...
      LEFT JOIN (
        SELECT t.response_id,
               t.comment,
               t.user,
               MAX(t.ctime) as ctime
        FROM tracking_info_tab t
        GROUP BY t.response_id
      ) c ON b.response_id = c.response_id
      INNER JOIN chatbi_topic d ON b.topic_id = d.id
        AND d.deleted_at = 0
      WHERE b.topic_id in :topicIds
        AND a.question_id != 0
        AND (:teamCode IS NULL OR d.team_code = :teamCode)
        AND (:messageId IS NULL OR a.response_id = :messageId)
        ...
        AND (:startTime IS NULL OR a.message_ctime >= :startTime)
        AND (:endTime IS NULL OR a.message_ctime <= :endTime)
      """, nativeQuery = true)
  List<Map<String, Object>> queryFeedbackDownloadData(
...
/**
  ...
  public FeedbackDownloadDTO downloadFeedbackRecords(FeedbackDownloadRequest request) {
    ...
    // 调用 permissionCheckService 获取要下载的 topic list 中符合权限要求的topic，
    // 并不是之前要求的用户必须拥有 topic list 中所有 topic 的权限
    List<Long> filterTopicIds = permissionCheckService.getOwnerOrProjectAdminOfTopic(resultTopicIds, currentUserEmail)
        .stream()
        .map(BaseDao::getId)
        .toList();
    ...
    List<Map<String, Object>> feedbackRecords = feedbackCycleRepository
        .queryFeedbackDownloadData(
            ...
            request.getSessionId(),
            request.getTeamCode(),
            request.getStatus() != null ? request.getStatus() : Collections.emptyList(),
            request.getFeedback() == null || request.getFeedback().equals("All sessions")
                ? null
                : feedbackResult,
            request.getUserName(),
    ...
    try (PrintWriter writer = new PrintWriter(
      ...
      for (FeedbackDownloadDao record : feedbackDownloadRecords) {
        ...
        writer.printf("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n",
            ...
            CsvUtils.escapeCSVField(record.getStatus()),
            CsvUtils.escapeCSVField(record.getComment()),
            CsvUtils.escapeCSVField(
                JsonUtils.fromJsonString(record.getQuestion(), RequestMessageDTO.class)
                    .getQuestion()),
            CsvUtils.escapeCSVField(
   ...
   * 将从数据库中查询出的哈希表映射为 FeedbackDownloadDao
   *
   * @param map SQL 查询结果映射
   * @return FeedbackDownloadDao 对象
   */
  private FeedbackDownloadDao mapToFeedbackDownloadDao(Map<String, Object> map) {
    return FeedbackDownloadDao.builder()
        .messageId((Long) map.getOrDefault("messageId", ""))
        .sessionId((Long) map.get("sessionId"))
        .topicName((String) map.get("topicName"))
        .status((String) map.get("status"))
        .comment((String) map.getOrDefault("comment", ""))
        .question((String) map.get("question"))
        .answer((String) map.getOrDefault("answer", ""))
        .feedbackResult((String) map.get("feedback"))
        .feedbackReason((String) map.get("feedbackReason"))
        .user((String) map.get("user"))
        .time((Long) map.getOrDefault("ctime", null))
        .build();
  }
/**
